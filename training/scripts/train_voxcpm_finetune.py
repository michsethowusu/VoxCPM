...
def train(
    ...
    if train_manifest == "ghana-latents":
        ...
        import sys as _sys
        _sys.path.insert(0, "/mnt/volume_d2wey28/projects/voxcpm-ghana")
        from voxcpm_ghana_data import build as _build_ghana
        train_ds, val_ds, _ghana_did = _build_ghana(dev_per_lang=40)
        print(f"[ghana] train={len(train_ds)} val={len(val_ds)} langs={len(_ghana_did)}", file=sys.stderr)
    else:
    ...
    # Precomputed-latent path: pack the batch in the DataLoader workers (CPU,
    # parallel, overlapped with GPU) instead of serially on the GPU. Pass the
    # packer config so build_dataloader can wire the worker-side collate.
    pack_config = dict(
        dataset_cnt=dataset_cnt,
        max_len=base_model.config.max_length,
        patch_size=base_model.config.patch_size,
